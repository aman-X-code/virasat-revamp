...
const getCloudinaryImageUrl = (folder: string, publicId: string, width: number = 600, rotation?: string) => {
  const transformations = [`f_auto`, `q_80`, `w_${width}`, `c_limit`];
  if (rotation) {
    transformations.push(rotation);
  }
  return `https://res.cloudinary.com/digilabs/image/upload/${transformations.join(',')}/prod/${folder}/prod/${folder}/${publicId}`;
};

const getCloudinaryVideoUrl = (folder: string, publicId: string) => {
  return `https://res.cloudinary.com/digilabs/video/upload/f_auto,q_auto/prod/${folder}/prod/${folder}/${publicId}`;
};

// Generate all gallery items dynamically - ONLY Cloudinary gallery images
const generateGalleryItems = () => {
  const items = [];
  
  // 2024 Gallery Images (16 images from Cloudinary)
  for (let i = 1; i <= 16; i++) {
    // Apply rotation correction for specific rotated images
    let rotation = undefined;
    if (i === 3 || i === 5 || i === 11) {
      rotation = 'a_-90'; // Rotate 90 degrees counter-clockwise to correct left rotation
    }
    items.push({ src: getCloudinaryImageUrl('gallery/2024', i.toString(), 500, rotation), type: 'photo', year: 2024 });
  }
  
  // 2023 Gallery Images (13 images from Cloudinary)
  for (let i = 1; i <= 13; i++) {
    // Apply rotation correction for specific rotated images
    let rotation = undefined;
    if (i === 7) {
    ...
    items.push({ src: getCloudinaryImageUrl('gallery/2023', i.toString(), 500, rotation), type: 'photo', year: 2023 });
  }
  
  // 2022 Gallery Images (14 images from Cloudinary)
  for (let i = 1; i <= 14; i++) {
    // Apply rotation correction for specific rotated images
    let rotation = undefined;
    if (i === 5 || i === 9) {
    ...
    items.push({ src: getCloudinaryImageUrl('gallery/2022', i.toString(), 500, rotation), type: 'photo', year: 2022 });
  }
  
  // 2021: No gallery folder created yet - empty
...
const GalleryContent = () => {
  ...
  const filteredItems = allItems.filter(item => {
    const yearMatch = filter.year === 'all' || item.year.toString() === filter.year.toString();
              ...
              >
                ...
                {item.type === 'photo' ? (
                  <div className="relative w-full h-full overflow-hidden">
                    <Image 
                      src={item.src} 
                      alt={`Gallery image from ${item.year}`} 
                      width={500} 
                      height={500} 
